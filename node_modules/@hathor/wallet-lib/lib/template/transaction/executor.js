...
exports.execChangeInstruction = execChangeInstruction;
...
async function execUtxoSelectInstruction(interpreter, ctx, ins) {
  ...
  const {
    changeAmount,
    utxos
  } = await interpreter.getUtxos(fill, options);
  ...
  if (autoChange && changeAmount) {
    // get change address
    let changeAddress = (0, _instructions.getVariable)(ins.changeAddress, ctx.vars, _instructions.UtxoSelectInstruction.shape.changeAddress);
    if (!changeAddress) {
 ...
 */
async function execChangeInstruction(interpreter, ctx, ins) {
  ctx.log(`Begin ChangeInstruction: ${_bigint.JSONBigInt.stringify(ins)}`);
  const token = (0, _instructions.getVariable)(ins.token, ctx.vars, _instructions.ChangeInstruction.shape.token);
  const address = (0, _instructions.getVariable)(ins.address, ctx.vars, _instructions.ChangeInstruction.shape.address) ?? (await interpreter.getChangeAddress(ctx));
  const timelock = (0, _instructions.getVariable)(ins.timelock, ctx.vars, _instructions.ChangeInstruction.shape.timelock);
  ...
  for (const tokenUid of tokensToCheck) {
    ...
    if (balance.tokens > 0) {
      ...
      const output = new _output.default(value, script, {
        timelock,
        tokenData
      });
      ctx.addOutputs(-1, output);
